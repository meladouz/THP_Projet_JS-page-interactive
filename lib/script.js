...
function parentNodeCard(){
  var parentNodeCard = document.getElementsByClassName("row");
  return parentNodeCard;
};

function selectBtnGris(){
  var selectBtnGris = document.getElementsByClassName("my-2")[1];
  return selectBtnGris
}
...
function oneCard() {
 ...
 selectBtnGris().addEventListener("click", function(){
   parentNodeCard()[1].insertBefore(oneCard()[(oneCard().length -1)], oneCard()[0]) ;
 });

 // ------------------------
 // fonctionnalité 8 - l'autre bouton
function selectBtnBleu(){
  var selectBtnBleu = document.getElementsByClassName("my-2")[0];
  selectBtnBleu.href = "#"
  return selectBtnBleu
}

selectBtnBleu().addEventListener("click",function(){
  parentNodeCard()[1].insertBefore(oneCard()[0], oneCard()[(oneCard().length)]) ;
})
